...
class RedisAdmissionStateStoreTest {
    ...
    void join_script_keeps_global_public_state_out_of_hot_path() {
        ...
        assertThat(script).contains("redis.call('SET', KEYS[6], '1', 'PX', marker_ttl_millis, 'NX')");
    }
    ...
    void advance_script_reads_tail_sequence_from_shard_counter() {
        ...
        assertThat(script).contains("local tail_seq = tonumber(redis.call('GET', KEYS[5]) or '0')");
        assertThat(script).contains("'tailSeq'");
        assertThat(script).contains("redis.call('PEXPIRE', KEYS[1], ttl_millis)");
    }

    @Test
    void enter_script_checks_existing_admission_before_ticket_lookup() {
        String script = RedisScriptLoader.load("redis/enter_queue.lua");

        assertThat(script.indexOf("local existing_token =")).isLessThan(script.indexOf("local ticket_value ="));
        assertThat(script).contains("string.find(KEYS[2], ':entered:', 1, true)");
        assertThat(script).contains("string.sub(KEYS[2], queue_id_start + 9)");
    }
    ...
    @Test
    @SuppressWarnings("unchecked")
    void joinQueue_does_not_register_scheduler_work_when_shard_marker_already_exists() {
        RedissonClient redissonClient = mock(RedissonClient.class);
        RScript script = mock(RScript.class);
        RSet<String> waitingPerformanceSet = mock(RSet.class);
        RedisAdmissionStateStore store = new RedisAdmissionStateStore(redissonClient);
    ...
    void enterQueue_uses_shard_local_keys_and_local_sequence() {
        ...
        assertThat(keysCaptor.getValue())
                .containsExactly(
                        ...
                        RedisKey.shardEntered(1L, 17, "queue-1"),
                        RedisKey.shardSessions(1L, 17),
                        RedisKey.shardQueue(1L, 17, "queue-1")
                );
        assertThat(argsCaptor.getValue())
    ...
    void advancePublicState_uses_event_lock_and_updates_projection() throws InterruptedException {
        ...
        when(script.evalSha(
                ...
                any(List.class),
                any(Object[].class)
        )).thenReturn(List.of(0L, 1L, 0L, 0L, 1L));
