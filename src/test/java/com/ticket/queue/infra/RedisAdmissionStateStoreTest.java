...
class RedisAdmissionStateStoreTest {
    ...
    @Test
    void join_script_keeps_public_state_out_of_the_hot_path() {
    ...
    @Test
    void advance_script_reads_tail_sequence_from_counter() {
        String script = RedisScriptLoader.load("redis/advance_queue_state.lua");

        assertThat(script).contains("redis.call('GET', KEYS[3])");
    ...
    void public_queue_keys_use_q_prefix_and_performance_hash_tag() {
        ...
        assertThat(RedisKey.publicWaitingMarker(1L)).isEqualTo("q:{1}:waiting-marker");
    ...
    void joinQueue_runs_lua_script_and_registers_performance_for_scheduler() {
        ...
        when(script.evalSha(
                ...
                any(List.class),
                any(Object[].class)
        )).thenReturn(List.of("queue-1", 42L, 1L, 1L));
        ...
        verify(script).scriptLoad(anyString());
    ...
    @Test
    @SuppressWarnings("unchecked")
    void joinQueue_does_not_register_scheduler_work_when_marker_already_exists() {
        RedissonClient redissonClient = mock(RedissonClient.class);
        RScript script = mock(RScript.class);
        RSet<String> waitingPerformanceSet = mock(RSet.class);
        RedisAdmissionStateStore store = new RedisAdmissionStateStore(redissonClient);
        ...
        when(script.evalSha(
                ...
                any(List.class),
                any(Object[].class)
        )).thenReturn(List.of("queue-2", 43L, 1L, 0L));
        ...
        assertThat(actual.created()).isTrue();
        verify(waitingPerformanceSet, org.mockito.Mockito.never()).add("1");
    }
    ...
    void advancePublicState_increments_admitted_sequence_with_lua_script() throws InterruptedException {
        ...
        store.advancePublicState(1L, 500, 5_000, Duration.ofHours(24));
        ...
        assertThat(argsCaptor.getValue()).containsExactly(500, 5_000, 86_400_000L);
        verify(lock).unlock();
    }

    @Test
    @SuppressWarnings("unchecked")
    void advancePublicState_keeps_scheduler_registration_when_waiting_marker_is_alive() throws InterruptedException {
        RedissonClient redissonClient = mock(RedissonClient.class);
        RScript script = mock(RScript.class);
        RLock lock = mock(RLock.class);
        RMap<String, String> stateMap = mock(RMap.class);
        RSet<String> waitingPerformanceSet = mock(RSet.class);
        RBucket<String> waitingMarker = mock(RBucket.class);
        RedisAdmissionStateStore store = new RedisAdmissionStateStore(redissonClient);
        ...
        when(redissonClient.<String, String>getMap(RedisKey.publicState(1L), StringCodec.INSTANCE))
                .thenReturn(stateMap);
        when(stateMap.readAllMap()).thenReturn(Map.of("admittedUntilSeq", "1000", "tailSeq", "1000"));
        when(redissonClient.<String>getSet(RedisKey.waitingPerformances(), StringCodec.INSTANCE))
                .thenReturn(waitingPerformanceSet);
        when(redissonClient.<String>getBucket(RedisKey.publicWaitingMarker(1L), StringCodec.INSTANCE))
                .thenReturn(waitingMarker);
        when(waitingMarker.isExists()).thenReturn(true);

        store.advancePublicState(1L, 500, 5_000, Duration.ofHours(24));

        verify(waitingPerformanceSet).remove("1");
        verify(waitingPerformanceSet).add("1");
    }
}
